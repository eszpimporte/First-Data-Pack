...
## Otherwise success
title @s actionbar {\
  "translate": "hygrave.change_config_message.graves.xp_distribution.grave_generation_fail.remove.success",\
  "fallback": "Successfully changed the 'remove' operation weight from %s to %s.",\
  "with": [\
    {"storage": "hygrave:common", "nbt": "configs.value.graves.xp_distribution.grave_generation_fail.remove"},\
