...
## Otherwise success
title @s actionbar {\
  "translate": "hygrave.change_config_message.graves.xp_distribution.grave_generation_success.keep.success",\
  "fallback": "Successfully changed the 'keep' operation weight from %s to %s.",\
  "with": [\
    {"storage": "hygrave:common", "nbt": "configs.value.graves.xp_distribution.grave_generation_success.keep"},\
