#@> Executed from:
#@>   function hygrave:internal/config/change/graves/item_distribution/grave_generation_success/remove/ench_ids/remove_ench_index
...
$execute if score .ench_id_list_is_valid hygrave.temp_var matches 1 run title @s actionbar {\
  "translate": "hygrave.change_config_message.item_distribution.grave_generation_success.remove.remove_ench_index.success",\
  "fallback": "Successfully removed enchantment %s from the list.",\
  "with": [\
    {\
      "nbt": "configs.value.graves.item_distribution.grave_generation_success.remove_enchs[$(index)]",\
