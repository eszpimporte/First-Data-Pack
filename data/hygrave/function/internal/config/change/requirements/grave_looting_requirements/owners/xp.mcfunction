...
execute unless predicate {\
  ...
  "translate": "hygrave.change_config_message.grave_looting_requirements.owners.xp.fail",\
  "fallback": "§cValue must be a non-negative integer."\
}

## Otherwise change values
execute store result score (requirements/grave_looting_requirements/owners/xp) hygrave.config run data get storage hygrave:common temp.config.levels
