#@> Executed from:
#@>   function hygrave:internal/config/change/requirements/grave_looting_requirements/owners/item_ids/remove_item_index

## Error if item does not exist
$execute if data storage hygrave:common configs.value.requirements.grave_looting_requirements.owners.item_ids[$(index)] run scoreboard players set .item_id_list_is_valid hygrave.temp_var 1

execute unless score .item_id_list_is_valid hygrave.temp_var matches 1 run title @s actionbar {\
  "translate": "hygrave.change_config_message.grave_looting_requirements.owners.remove_item_index.fail.no_exist",\
