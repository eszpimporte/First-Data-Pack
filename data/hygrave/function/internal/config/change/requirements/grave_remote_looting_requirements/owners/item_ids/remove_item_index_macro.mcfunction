...
execute unless score .item_id_list_is_valid hygrave.temp_var matches 1 run title @s actionbar {\
  "translate": "hygrave.change_config_message.grave_remote_looting_requirements.owners.remove_item_index.fail.no_exist",\
...
$execute if score .item_id_list_is_valid hygrave.temp_var matches 1 run title @s actionbar {\
  "translate": "hygrave.change_config_message.grave_remote_looting_requirements.owners.remove_item_index.success",\
  "fallback": "Successfully removed item %s from the list.",\
  "with": [\
    {\
      "nbt": "configs.value.requirements.grave_remote_looting_requirements.owners.item_ids[$(index)]",\
...
$execute if score .item_id_list_is_valid hygrave.temp_var matches 1 run data remove storage hygrave:common configs.value.requirements.grave_remote_looting_requirements.owners.item_ids[$(index)]