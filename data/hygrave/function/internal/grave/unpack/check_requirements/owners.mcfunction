...
execute if data storage hygrave:common configs.value.requirements.grave_looting_requirements.owners.item_ids[0] run function hygrave:internal/grave/unpack/check_requirements/owners/items with storage hygrave:common configs.value.requirements.grave_looting_requirements.owners
execute if score .check_requirements.items hygrave.temp_var matches 1 if data storage hygrave:common configs.value.requirements.grave_looting_requirements.owners.item_ids[0] run function hygrave:internal/grave/unpack/check_requirements/owners/items/remove with storage hygrave:common configs.value.requirements.grave_looting_requirements.owners
