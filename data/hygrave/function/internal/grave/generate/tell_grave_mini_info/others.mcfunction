...
execute unless score (graves/show_grave_info/an/location.xyz) hygrave.config matches 1 unless score (graves/show_grave_info/an/location.dim) hygrave.config matches 1 run tellraw @a[tag=!hygrave.temp.grave.owner] {\
  "translate": "\n§c☠ %s §6#%s\n",\
  "with": [\
    {\
      "nbt": "item.components.minecraft:custom_data.hygrave:common.owner.name",\
